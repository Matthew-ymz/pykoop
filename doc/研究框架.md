...
=\frac{1}{r}\sum_{i=1}^r \sigma_i^\alpha-\frac{1}{d_{\mathrm{eff}}}\sum_{i=1}^{d_{\mathrm{eff}}}\sigma_i^\alpha.
\tag{6.5}
$$
...
这一部分对应仓库中已经实现的三类实验：Kuramoto 实验、Rulkov map 实验和空气质量实验。前三部分主要给出理论基础，这里则把理论如何落到代码、实验如何组织、参数如何扫描、结果如何解释讲清楚，方便没有看过代码的人直接理解整个实验链条。

### 9.1 代码框架

三类实验虽然数据来源不同，但主分析链条是一致的。统一流程可以概括为：

1. 生成或读取微观时间序列数据，构成状态矩阵 $X_t$。
2. 选择观测函数并做提升，得到观测空间中的轨道矩阵。
3. 基于滞后配对样本构造协方差矩阵 $C_{00}, C_{01}, C_{11}$。
4. 拟合原始 Koopman 矩阵 $K_{\mathrm{raw}}$，并进一步构造双边白化矩阵 $\bar K$。
5. 对 $\bar K$ 做奇异值分解，得到奇异值谱、左奇异向量和粗粒化系数。
6. 计算主总分、单通道得分、宏观效率增益 CE、EC 值以及最优截断维数 $r$。
7. 构造宏观变量，并对宏微观结构进行比较解释。

为方便理解，下面按“输入是什么、做什么、输出什么”的方式说明核心函数。

#### 9.1.1 通用核心函数

| 函数 | 输入 | 计算内容 | 输出 |
| --- | --- | --- | --- |
| `compute_transition_covariances(sample_points_list, library=None, weights=None, lag_steps=1)` | 一条或多条时间序列；可选观测函数；滞后步长 | 对每条序列构造 $(X_t, X_{t+\tau})$ 配对样本，并计算协方差矩阵 $C_{00}, C_{01}, C_{11}$ | 字典，含 `C00`、`C01`、`C11`、配对样本 `X/Y`、权重、配对数 |
| `fit_data_koopman_operator(sample_points_list, library=None, weights=None, eps=1e-10, ridge=0.0, lag_steps=1)` | 与上相同，另加正则和截断参数 | 先拟合原始一步传播矩阵，再通过双边白化得到 $\bar K$ | 字典，含 `A`、`K_bar`、`K_bar_from_A`、`C00/C01/C11`、平方根与逆平方根矩阵 |
| `analyze_kbar_metrics(K_bar, alpha=1.0, eps=1e-10)` | 白化矩阵 $\bar K$ | 对 $\bar K$ 做 SVD，计算奇异值谱、有效秩、残差算子 $D_K$、增强算子 $N_K$、单通道得分 $g_i$ 以及主总分 $G_\alpha^K$ | 字典，含 `U`、`S`、`Vt`、`effective_rank`、`D_K`、`N_K`、`channel_scores`、`G_alpha_K` |
| `get_positive_contributions(singular_values)` | 奇异值谱 | 计算 EC 谱所需的正贡献序列 | EC 谱序列 |
| `compute_entropy(diff)` | EC 谱序列 | 计算该谱对应的熵型指标 | 单个 `EC` 值 |
| `build_macro_from_kbar(X, feature_names, U, r)` | 提升后的微观观测矩阵、特征名、左奇异向量、截断维数 | 用左奇异向量前 $r$ 列构造粗粒化系数和宏观变量 | 字典，含宏观变量矩阵、粗粒化系数、重要特征排序 |
| `print_summary(summary_dict)` | 实验摘要字典 | 把关键维度、协方差、主总分和 CE 信息结构化打印 | 格式化文本摘要 |

#### 9.1.2 通用指标说明

为了后文三类实验读起来一致，这里统一说明几个常用指标。

| 指标 | 含义 | 作用 |
| --- | --- | --- |
| `G_alpha_K` | 基于 $\bar K$ 的主总分 | 描述整体主导通道结构的强弱 |
| `g_i` | 单通道得分 | 描述第 $i$ 个奇异通道对宏观效率的贡献 |
| `delta_g_by_r` | 对每个截断维数 $r$，比较前 $r$ 个通道与后续通道的收益差 | 用来选择最优宏观维数 |
| `selected_r` 或 `r` | 使 `delta_g_by_r` 最大的维数 | 作为宏观变量个数 |
| `CE` | `delta_g_selected_r`，即最优 $r$ 下的宏观效率增益 | 衡量宏观化后效率提升的幅度 |
| `EC` | 基于奇异值谱的熵型指标 | 描述主导通道分布的集中度或复杂度 |
| `effective_rank` | 大于阈值的有效奇异值数量 | 描述系统在当前表示下的有效维度 |

其中，三个实验里都使用了 `CE` 这个记号，统一指最优截断维数对应的宏观效率增益。

#### 9.1.3 各实验的特殊函数与特殊模块

虽然主链一致，但不同实验在数据入口和结果解释上有明显差别。

**Kuramoto 实验的特殊部分**

| 函数或模块 | 作用 |
| --- | --- |
| `generate_kuramoto_cluster_data_sin_cos(...)` | 生成双群体 Kuramoto 微观数据，并直接给出正余弦嵌入状态 |
| 频谱分析模块 | 对代表性微观振子和宏观变量做频谱分析，用来比较主频是否与真实群体频率一致 |
| 互信息模块 | 计算 `I(Z_t; X_{t+1})`、`I(Z_{eig,t}; X_{t+1})`、`I(X_t; X_{t+1})`，比较宏观变量与微观未来的统计耦合强度 |
| `best_cluster_corr` 相关模块 | 在 sweep 中评价两个宏观变量是否能够一一对应两个真实微观团 |

**Map 实验的特殊部分**

| 函数或模块 | 作用 |
| --- | --- |
| `generate_two_population_neuron_data(...)` | 生成双群体 Rulkov 神经元数据 |
| `extract_state_matrix_from_rulkov_data(...)` | 从原始 Rulkov 字典中抽取统一的状态矩阵、状态名、时间轴和同步指标 |
| `build_map_comparison_table(...)` | 把四种状态 `CS/GS/Q/D` 的指标整理成统一表格 |
| 四状态 CE/EC 对比模块 | 在统一分析参数下比较四种状态的 CE 和 EC，用于观察不同同步结构下宏观效率的差异 |

**Air 实验的特殊部分**

| 函数或模块 | 作用 |
| --- | --- |
| `load_air_data_subset(...)` | 从全部站点数据中选择“全部/某省/某市”的子集 |
| `build_air_feature_matrix(...)` | 根据所选变量和站点构造微观状态矩阵 |
| `lift_time_delay(...)` | 用时间延迟嵌入代替 Fourier 等观测函数，把历史记忆纳入扩展状态 |
| `summarize_air_subset(...)` | 汇总所选子集的站点数、地区范围、时间长度等信息 |
| `plot_station_with_map(...)` | 把左奇异向量或粗粒化系数投影到站点地图上，显示空间分布结构 |

因此，三类实验可以理解为：理论主链相同，但“数据入口、观测函数和结果解释方式”分别针对不同系统做了定制。

### 9.2 Kuramoto 实验

Kuramoto 实验对应文件：

- 主 notebook：`exp/kuramoto/exp_kuramoto_0321.ipynb`
- 参数实验目录：`exp/kuramoto/para_exp/`

该实验研究的是双群体 Kuramoto 模型。微观层有两个真实振子团，因此参数扫描时将宏观变量目标数固定为 2，用来直接评价“两个宏观变量能否对应两个真实团体结构”。

#### 9.2.1 参数扫描总览

Kuramoto 的参数实验主要扫描分析和生成中的关键耦合参数，重点是：

| 参数 | 说明 |
| --- | --- |
| `K_intra` | 群体内耦合强度。本轮扫描固定为 `5.0` |
| `K_inter` | 群体间耦合强度，是本轮的主要扫描对象 |
| `lag_steps` | Koopman 配对步长，是本轮的另一主要扫描对象 |
| `sample_stride` | 下采样间隔，本轮固定为 `10` |
| `library_kind` | 观测函数形式，本轮固定为 `identity+fourier` |
| `rank_target` | 宏观变量个数，本轮固定为 `2`，与真实双群体一致 |

参数实验文件如下：

| 文件 | 用途 |
| --- | --- |
| `exp/kuramoto/para_exp/sweep_kinter_lag_configs.json` | 粗扫描参数清单 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_refine_configs.json` | 细扫描参数清单 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_results.jsonl` | 粗扫描详细结果 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_refine_results.jsonl` | 细扫描详细结果 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_summary.csv` | 参数扫描总表 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_top10.csv` | Top 10 结果 |
| `exp/kuramoto/para_exp/sweep_kinter_lag_summary.md` | 文字摘要 |

#### 9.2.2 参数扫描结果表

本轮扫描得到的最佳结果如下。

| run_name | K_intra | K_inter | lag_steps | sample_stride | library_kind | score | best_cluster_corr | near_one_count_099 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `refine_Kinter_0p070_lag_12` | 5.0 | 0.07 | 12 | 10 | `identity+fourier` | -0.0492 | 0.9470 | 21 |

Top 结果的局部区间主要集中在：

| 参数区域 | 观察 |
| --- | --- |
| `K_inter ≈ 0.07~0.085` | 最优点集中在这一小段区间 |
| `lag_steps ≈ 12~15` | 中等预测步长表现最好 |

#### 9.2.3 指标解释

Kuramoto 参数实验使用了一套专门针对“是否恢复双群体结构”的评分逻辑。

| 指标 | 含义 | 作用 |
| --- | --- | --- |
| `best_cluster_corr` | 在最佳排列匹配下，两个宏观变量与两个真实微观团的平均相关性 | 越接近 1，说明宏观变量越像真实双群体结构 |
| `near_one_count_099` | 大于 0.99 的奇异值个数 | 表示接近无损通道的数量 |
| `score` | 综合评分，结合谱间隙、团体相关性、频率匹配和近 1 奇异值数 | 用来给参数组合排序 |

这里的 `best_cluster_corr` 是在 `2 × 2` 相关性矩阵上，考虑宏观变量顺序可交换后得到的最佳平均相关性。其核心目的不是比较宏观变量越多越好，而是检查“两个宏观变量是否能正确对应两个真实团体”。

#### 9.2.4 重要图示

| 图片 | 横轴 | 纵轴 | 内容 | 作用 | 结论 | 路径 |
| --- | --- | --- | --- | --- | --- | --- |
| `kuramoto_sweep_heatmap.png` | `lag_steps` | `K_inter` | 热图颜色表示 `score` | 快速定位最佳参数区间 | 最优区域集中在 `K_inter≈0.07~0.085`、`lag≈12~15` | `exp/kuramoto/para_exp/kuramoto_sweep_heatmap.png` |
| `kuramoto_top_runs.png` | `K_inter` | `lag_steps` | Top 结果散点图，颜色表示 `best_cluster_corr` | 对热点区域做局部放大 | 最优点在中等群体间耦合和中等步长附近聚集 | `exp/kuramoto/para_exp/kuramoto_top_runs.png` |

#### 9.2.5 与主 notebook 的关系

参数实验找到的最佳组合已经用于 notebook 配置回填：
...
在这一配置下，notebook 中可继续看到更完整的链条输出，包括协方差矩阵、奇异值谱、互信息分析、频谱分析和宏观变量解释。

### 9.3 Maps 实验

Map 实验对应文件：

- 主 notebook：`exp/discrete_maps/exp_map_0321.ipynb`
- 参数实验目录：`exp/discrete_maps/para_exp/`

该实验使用双群体 Rulkov 神经元系统，并分别生成四种典型状态：
...
本轮参数实验**不调数据生成参数**，而是固定四种状态的数据，只调分析链参数，以保证四种状态在统一分析条件下可比较。
...
| 文件 | 用途 |
| --- | --- |
| `exp/discrete_maps/para_exp/map_analysis_sweep_configs.json` | 分析参数搜索空间 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_results.jsonl` | 每组参数的详细结果 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_history.jsonl` | 运行历史 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_summary.csv` | 参数组合总表 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_top10.csv` | Top 10 参数组合 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_state_metrics.csv` | 所有组合下四状态逐状态指标 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_best_comparison.csv` | 最佳参数下四状态比较表 |
| `exp/discrete_maps/para_exp/map_analysis_sweep_summary.md` | 文字摘要 |

#### 9.3.2 最优分析参数

| run_name | lag_steps | sample_stride | ridge | alpha | avg_CE | avg_EC |
| --- | --- | --- | --- | --- | --- | --- |
| `map_lag1_stride2_ridge1e-10_alpha0.5` | 1 | 2 | `1e-10` | 0.5 | 2954.7118 | 7.0344 |

该最优参数的含义是：在当前四状态数据和当前代码实现下，最短步长的 Koopman 配对配合适度下采样，能给出最稳定的统一比较。

#### 9.3.3 四状态结果表

最佳参数下，四种状态的核心结果如下。

| 状态 | 同步状态判定 | EC | r | CE | effective_rank | 说明 |
| --- | --- | --- | --- | --- | --- | --- |
| `CS` | Complete Synchronization | 6.4704 | 1 | 3427.9930 | 26 | 完全同步下最优宏观维数极低，说明整体结构高度集中 |
| `GS` | Generalized Synchronization | 6.6032 | 4 | 3418.8970 | 36 | 仍具有较高 CE，但需要略多宏观通道 |
| `Q` | Chimera State | 8.2987 | 63 | 2982.6771 | 213 | EC 明显更高，表明谱结构更复杂 |
| `D` | Desynchronization | 6.7653 | 273 | 1989.2801 | 400 | 需要更多宏观维数，宏观效率增益明显下降 |

这里的 `r` 是通过 `delta_g_by_r` 选出来的最优截断维数，`CE` 就是该 `r` 对应的 `delta_g_selected_r`。

#### 9.3.4 重要图示

| 图片 | 横轴 | 纵轴 | 内容 | 作用 | 结论 | 路径 |
| --- | --- | --- | --- | --- | --- | --- |
| `map_sweep_best_ce.png` | `CS/GS/Q/D` | `CE` | 四状态在统一最优分析参数下的宏观效率增益 | 直接比较四状态的 CE | `CS` 和 `GS` 最高，`D` 最低 | `exp/discrete_maps/para_exp/map_sweep_best_ce.png` |
| `map_sweep_best_ec.png` | `CS/GS/Q/D` | `EC` | 四状态在统一最优分析参数下的 EC | 比较谱复杂度差异 | `Q` 的 EC 最显著 | `exp/discrete_maps/para_exp/map_sweep_best_ec.png` |

#### 9.3.5 如何理解该实验

Map 实验的核心不是找“最强同步状态”，而是比较：

- 不同同步结构下，主导通道如何分布；
- 宏观效率增益 CE 如何变化；
- 最优宏观维数 $r$ 如何随状态改变。

因此，Map 实验更像是一组“同一分析链下的状态比较实验”。

### 9.4 Air 实验

Air 实验对应文件：

- 主 notebook：`exp/air_quality/exp_air_0321.ipynb`
- 参数实验目录：`exp/air_quality/para_exp/`

该实验用真实空气质量数据代替模拟微观数据。与前两个实验不同，Air 的重点不在预设团体结构，而在于比较不同空间范围和时间延迟设置下，白化 Koopman 主链得到的宏观效率差异。
...
| --- | --- |
| 省级 | 上海市、江苏省、浙江省 |
| 市级 | 上海市、南京市、南通市、台州市、嘉兴市、宁波市、常州市、扬州市、无锡市、杭州市、泰州市、湖州市、绍兴市、舟山市、苏州市、镇江市 |

#### 9.4.2 参数实验文件

| 文件 | 用途 |
| --- | --- |
| `exp/air_quality/para_exp/air_sweep_configs.json` | 全部参数配置 |
| `exp/air_quality/para_exp/air_sweep_results_utf8.jsonl` | 有效运行结果明细 |
| `exp/air_quality/para_exp/air_sweep_history_utf8.jsonl` | 有效运行历史 |
| `exp/air_quality/para_exp/air_sweep_summary.csv` | 总汇总表 |
| `exp/air_quality/para_exp/air_sweep_top10.csv` | Top 10 结果 |
| `exp/air_quality/para_exp/air_sweep_group_summary.csv` | 按范围和步长聚合的汇总 |
| `exp/air_quality/para_exp/air_sweep_summary.md` | 文字摘要 |

#### 9.4.3 全局最优结果

| run_name | scope | delay_days | lag_steps | station_count | feature_dim | selected_r | CE | EC |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `air_all_delay3_lag48` | `all` | 3 | 48 | 127 | 1016 | 2 | 6.3229 | 8.9481 |

这说明在全部站点范围内，3 天延迟、48 步预测配对下，当前实验链条能获得最强的宏观效率增益。

#### 9.4.4 重要汇总图

为了便于比较，本轮又补充了按“全部 + 每个省 + 每个市”逐实体汇总的图，图下注释写明固定参数和筛选规则。

| 图片 | 横轴 | 纵轴 | 内容 | 作用 | 结论 | 路径 |
| --- | --- | --- | --- | --- | --- | --- |
| `air_best_ce_by_scope.png` | `all/province/city` | 最佳 `CE` | 比较三层数据范围的最优 CE | 先看不同层级的总体上限 | 全部站点层级最高 | `exp/air_quality/para_exp/air_best_ce_by_scope.png` |
| `air_avg_ce_by_lag.png` | `lag_steps` | 平均 `CE` | 比较不同配对步长的平均表现 | 看哪种步长整体更稳 | `lag_steps=48` 最优 | `exp/air_quality/para_exp/air_avg_ce_by_lag.png` |
| `air_ce_by_scope_entity.png` | `all + 各省 + 各市` | `CE` | 每个实体取其 sweep 中 CE 最优的一次结果 | 看不同空间实体的最佳 CE 分布 | `all` 最高，省市之间有明显差异 | `exp/air_quality/para_exp/air_ce_by_scope_entity.png` |
| `air_ec_by_scope_entity.png` | `all + 各省 + 各市` | `EC` | 每个实体取其 CE 最优结果对应的 EC | 同步比较不同实体的谱复杂度 | 大范围数据通常 EC 更高 | `exp/air_quality/para_exp/air_ec_by_scope_entity.png` |
| `air_station_count_and_r_by_scope_entity.png` | `all + 各省 + 各市` | 左轴为站点数，右轴为 `r` | 用双 Y 轴比较空间规模和宏观维数 | 看数据规模是否影响最优宏观维数 | 当前汇总中 `r` 变化较小，但站点数差异明显 | `exp/air_quality/para_exp/air_station_count_and_r_by_scope_entity.png` |

#### 9.4.5 如何理解该实验

Air 实验中的“观测函数”不是 Fourier 展开，而是时间延迟嵌入。因此，Air 实验要回答的问题不是“团体结构是否恢复”，而是：

- 在不同空间范围下，延迟观测空间中的主导预测通道是否集中；
- 哪种时间尺度的配对步长最适合空气质量数据；
- 宏观变量能否用较少维数保留关键预测信息。

从这轮结果看，最稳定的结论是：

- 全部站点数据仍然保留最强的宏观效率增益；
- `lag_steps=48` 在三类范围中都有较强表现；
- 随着空间范围缩小，站点数下降，宏观效率增益也整体下降。

#### 9.4.6 与前两类实验的统一理解

虽然 Air 没有像 Kuramoto 那样预设两个微观团，也没有像 Map 那样固定四种同步状态，但三类实验仍然遵循同一研究问题：

> 在不同系统和不同观测表示下，白化 Koopman 主链能否提取高效、低维、可解释的宏观动态通道？

三类实验分别对应三种研究视角：

- Kuramoto：检验是否能恢复已知团体结构；
- Map：比较不同同步状态下宏观效率如何变化；
- Air：在真实数据中观察空间尺度和时间记忆对宏观变量的影响。
